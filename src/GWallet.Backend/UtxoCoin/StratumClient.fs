...
type public ElectrumServerReturningErrorInJsonResponseException (message: string,
                                                                 code: int) =
...
type StratumClient (jsonRpcClient: JsonRpcTcpClient) =
           ...
           && (not (Object.ReferenceEquals (maybeError.Error, null))) then
            raise
                (ElectrumServerReturningErrorInJsonResponseException
                    (maybeError.Error.Message, maybeError.Error.Code))
    ...
    member self.BlockchainScriptHashGetBalance address
    ...
    member self.ServerVersion (clientName: string) (protocolVersion: Version)
                              : Async<Version> =
        async {
            let obj =
                {
    ...
    member self.BlockchainScriptHashListUnspent address
    ...
    member self.BlockchainTransactionGet txHash
    ...
    member self.BlockchainEstimateFee (numBlocksTarget: int)
    ...
    member self.BlockchainTransactionBroadcast txInHex
