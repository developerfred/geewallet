...
module FSharpUtil =
    ...
    module ReflectionlessPrint =
        ...
        let ToStringFormat (fmt: string) =
            let rec inner (innerFmt: string) (count: uint32) =
                ...
                let formatsFound =
                    supportedFormats.Where
                        (fun format -> innerFmt.IndexOf (format) >= 0)

                if formatsFound.Any () then
                    let firstIndexWhereFormatFound =
                        formatsFound.Min
                            (fun format -> innerFmt.IndexOf (format))
    ...
    let ListIntersect<'T> (list1: List<'T>)
                          (list2: List<'T>)
                          (offset: uint32)
                          : List<'T> =
    ...
    let rec public FindException<'T when 'T :> Exception> (ex: Exception)
                                                          : Option<'T> =
