...
type ServerDiscardedException (message: string,
                               innerException: CommunicationUnsuccessfulException) =
    inherit Exception(message, innerException)

type BuggyExceptionFromOldMonoVersion (message: string,
                                       innerException: Exception) =
    inherit CommunicationUnsuccessfulException(message, innerException)

type ServerClosedConnectionEarlyException (message: string,
                                           innerException: Exception) =
    inherit CommunicationUnsuccessfulException(message, innerException)
...
module Networking =
    ...
    let FindExceptionToRethrow (ex: Exception)
                               (newExceptionMsg)
                               : Option<Exception> =
