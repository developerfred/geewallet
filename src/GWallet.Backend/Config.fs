...
module Config =
                    ...
                    for ethAccountFilePath in Directory.GetFiles
                        ...
                        let newPath =
                            ethAccountFilePath.Replace
                                (ethConfigDir.FullName, tokenConfigDir.FullName)

                        if not (File.Exists newPath) then
                            File.Copy (ethAccountFilePath, newPath)

    let GetAccountFiles (currencies: seq<Currency>)
    ...
    let AddAccount (conceptAccount: ConceptAccount)
                   (accountKind: AccountKind)
                   : FileRepresentation =
        ...
        let newAccountFile =
            Path.Combine
                (configDir.FullName, conceptAccount.FileRepresentation.Name)
            |> FileInfo
