...
module internal Account =
    ...
    let internal GetPublicAddressFromNormalAccountFile (accountFile: FileRepresentation)
                                                       : string =
    ...
    let internal GetAccountFromFile (accountFile: FileRepresentation)
        ...
        | AccountKind.ReadOnly ->
            ReadOnlyAccount
                (currency, accountFile, (fun accountFile -> accountFile.Name))
            :> IAccount
        | AccountKind.Normal ->
            NormalAccount
                (currency, accountFile, GetPublicAddressFromNormalAccountFile)
            :> IAccount
    ...
    let private GetTransactionCount (currency: Currency)
    ...
    let EstimateEtherTransferFee (account: IAccount)
    ...
    let EstimateTokenTransferFee (account: IAccount)
                                 amount
                                 destination
                                 : Async<TransactionMetadata> =
        async {
    ...
    let EstimateFee (account: IAccount)
                    (amount: TransferAmount)
                    destination
                    : Async<TransactionMetadata> =
        async {
    ...
    let private SignEtherTransaction (currency: Currency)
        ...
        let amountInWei =
            UnitConversion.Convert.ToWei
                (amountToSendConsideringMinerFee, UnitConversion.EthUnit.Ether)
    ...
    let private SignEtherTokenTransaction (currency: Currency)
        ...
        let amountInWei =
            UnitConversion.Convert.ToWei
                (amount.ValueToSend, UnitConversion.EthUnit.Ether)
    ...
    let private CreateInternal (password: string)
                               (seed: array<byte>)
                               : FileRepresentation =
